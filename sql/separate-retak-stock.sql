-- ═══════════════════════════════════════════════════
-- PISAHKAN STOK RETAK DARI BENTES
-- ═══════════════════════════════════════════════════
...
BEGIN
    ...
    LOOP
        ...
        IF rec.prod_data ? 'retak' THEN
            ...
            prod := jsonb_set(prod, '{Retak,butir}', (COALESCE((prod->'Retak'->>'butir')::int, 0) + b)::text::jsonb);
            prod := jsonb_set(prod, '{Retak,kilo}', (COALESCE((prod->'Retak'->>'kilo')::numeric, 0) + k)::text::jsonb);
        END IF;
