    ...
    LOOP
        ...
        LOOP
            ...
            IF prod ? g THEN
                ...
                prod := jsonb_set(prod, ARRAY[g, 'butir'], (GREATEST(0, (prod->g->>'butir')::int - b))::text::jsonb);
                prod := jsonb_set(prod, ARRAY[g, 'kilo'], (GREATEST(0, (prod->g->>'kilo')::numeric - k))::text::jsonb);
            END IF;
        END LOOP;
    END LOOP;
